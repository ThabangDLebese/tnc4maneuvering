...
from tnc4maneuvering.models import WFPCCEncoder
...
def main(args):
    ...
    encoder = WFPCCEncoder(encoding_size=64) # since it references the original inputs
    ...
    for i, path in enumerate(['one_ds']):
        ...
        for cv in range(1):
            ...
            plt.tight_layout()
            plt.savefig(os.path.join("./plots/%s" %path, "PCC-prunedKMeans_cent_dist.png"), dpi=100)
            plt.close()
            ...
            plt.tight_layout()
            plt.savefig(os.path.join("./plots/%s" %path, "PCC-prunedsse_KMeansClusters.png"), dpi=100)
            plt.close()
            ...
            plt.tight_layout()
            plt.savefig(os.path.join("./plots/%s" %path, "PCC-prunedCluster_points.png"), dpi=100)
            del encodings

        print('PCC-pruned Silhouette score: ', np.mean(s_score),'+-', np.std(s_score))
        print('PCC-pruned Davies Bouldin score: ', np.mean(db_score),'+-', np.std(db_score))
        print("PCC-pruned Intra-Cluster Distance:", np.mean(intra_cluster_distances),'+-', np.std(intra_cluster_distances))
        print("PCC-pruned Inter-Cluster Distance:", np.mean(inter_cluster_distances),'+-', np.std(inter_cluster_distances))
        print("PCC-pruned Calinski-Harabasz Index:", calinski_harabasz_index)
