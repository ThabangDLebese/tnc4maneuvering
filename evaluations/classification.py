...
from tnc4maneuvering.models import WFEncoder, WFClassifier
...
def train(train_loader, valid_loader, classifier, lr, data_type, encoder=None, n_epochs=500, type='e2e', cv=0): #500
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig(os.path.join("./plots/%s" % data_type, "classification_%s_%d.png"%(type, cv)), dpi=100)
    return best_acc, best_auc, best_aupc

def run_test(data, tnc_lr, data_path, window_size, n_cross_val):
    # Load data
    ...
    for cv in range(n_cross_val):
        ...
        if data == f'{args.data}': 
            encoding_size = 64  # Encodoer size
            tnc_encoder = WFEncoder(encoding_size=encoding_size).to(device)
            checkpoint_dir = './ckpt/%s/' % args.data
            if not os.path.exists(checkpoint_dir):
                os.makedirs(checkpoint_dir)

            checkpoint_path = os.path.join(checkpoint_dir, 'checkpoint_%d.pth.tar' % cv)
            if not os.path.exists(checkpoint_path):
                raise RuntimeError('Checkpoint for TNC4M encoder does not exist at path: %s' % checkpoint_path)

            tnc_checkpoint = torch.load(checkpoint_path)
        ...
        # Train and evaluate TNC model
        best_acc_tnc, best_auc_tnc, best_auprc_tnc = train(train_loader, valid_loader, tnc_classifier, tnc_lr,
                                                          encoder=tnc_encoder, data_type=data, n_epochs=n_epochs, type='tnc4man', cv=cv)
        ...
        with open("./ckpt/%s_classifiers.txt" % data, "a") as f:
            ...
            f.write("TNC4Maneuvering: \t AUC: %s\t Accuracy: %s \n\n" % (str(best_auc_tnc), str(100 * best_acc_tnc)))
        torch.cuda.empty_cache()

    print('=======> Performance Summary:')
    print('TNC4Maneuvering: \t Accuracy: %.2f +- %.2f \t AUC: %.3f +- %.3f \t AUPRC: %.3f +- %.3f' %
...
if __name__ == '__main__':
    ...
    f.close()    
    run_test(data=args.data, tnc_lr=0.01, data_path=data_path, window_size=args.window_size, n_cross_val=args.cv) 