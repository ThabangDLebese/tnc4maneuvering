...
def main(args):
    ...
    pca = PCA()
    pca.fit(all_reps_overtime[labels])
    eigenvalues = pca.explained_variance_
    plt.plot(range(1, len(eigenvalues) + 1), eigenvalues, marker='o')
    plt.grid(True)
    plt.xlabel('Principal Component')
    plt.ylabel('Eigenvalue')
    plt.title('Eigenvalue Ranking')
    plt.savefig(os.path.dirname(file_path) + f"/{args.dataset}_prunedPCreg_loss.png", dpi=100)

    explained_variance_ratio = pca.explained_variance_ratio_
    cumulative_variance_ratio = np.cumsum(explained_variance_ratio)
    n_components = np.argmax(cumulative_variance_ratio >= 0.95) + 1
    pca_selected = PCA(n_components=n_components)
    transformed_data = pca_selected.fit_transform(all_reps_overtime[labels])
    PCApruned_reps_overtime = pd.DataFrame(transformed_data, columns=['PC{}'.format(i) for i in range(1, n_components + 1)])
    ...
    def multivariate_linear_regression(x_train, y_train, x_test, y_test, lr=0.01, n_epochs=200000, patience=2000):
        ...
        ax.set_ylabel("Loss")
        ax.legend()
        plt.savefig(os.path.dirname(file_path) + f"/{args.dataset}_PCAprunedreg_loss.png", dpi=100)
        ...
        plt.savefig(os.path.dirname(file_path) + f"/{args.dataset}_PCAprunedpred_true.png", dpi=100)
    ...
    x = PCApruned_reps_overtime.T.to_numpy() # encodings
