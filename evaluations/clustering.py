...
def main(args):
    ...
    for i, path in enumerate(['one_ds']):
        ...
        for cv in range(1):
            ...
            plt.tight_layout()
            plt.savefig(os.path.join("./plots/%s" %path, "KMeans_cent_dist.png"), dpi=100)
            plt.close()
            ...
            plt.tight_layout()
            plt.savefig(os.path.join("./plots/%s" %path, "sse_KMeansClusters.png"), dpi=100)
            plt.close()
            ...
            plt.tight_layout()
            plt.savefig(os.path.join("./plots/%s" %path, "Cluster_points.png"), dpi=100)
            del encodings

        print('Silhouette score: ', np.mean(s_score),'+-', np.std(s_score))
        print('Davies Bouldin score: ', np.mean(db_score),'+-', np.std(db_score))
        print("Intra-Cluster Distance:", np.mean(intra_cluster_distances),'+-', np.std(intra_cluster_distances))
        print("Inter-Cluster Distance:", np.mean(inter_cluster_distances),'+-', np.std(inter_cluster_distances))
