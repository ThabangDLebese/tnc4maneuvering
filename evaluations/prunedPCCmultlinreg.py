...
# from tnc4maneuvering.models import WFEncoder
# from models import WFClassifier, WFPcaEncoder
# from tnc4maneuvering import prunedPCCclass
...
def main(args):
    ...
    # one_ds
    file_path = f'./plots/{args.dataset}/reps_overtime.parquet.gz'
    ...
    all_reps_overtime  = table.to_pandas()

    # Labeling the representations and perform PCA:
    labels = ['label{}'.format(i) for i in range(all_reps_overtime.shape[1])]
    all_reps_overtime.columns = labels

    # Calculate correlation matrix
    corr_matrix = all_reps_overtime[labels].corr().abs()
    to_drop = set()
    for i in range(len(corr_matrix.columns)):
        for j in range(i):
            if abs(corr_matrix.iloc[i, j]) > 0.8: # make this a parameter to be input in the command line
                to_drop.add(corr_matrix.columns[i])
    print('Dropped feature(s):', to_drop)
    PCCpruned_reps_overtime = all_reps_overtime.drop(to_drop, axis=1)

    # Assuming embed_trajectory_hm is the DataFrame containing the data
    print('Remaining non-correlated features: \n',PCCpruned_reps_overtime.columns)
    print('No. remaining non-correlated features:', len(PCCpruned_reps_overtime.columns))
    print('Remaining non-correlated shape: \n',PCCpruned_reps_overtime.shape)

    # Compute the correlation matrix
    correlation_matrix = PCCpruned_reps_overtime.corr()
    # Generate a mask for the upper triangle
    mask = np.triu(np.ones_like(correlation_matrix, dtype=bool))
    # Plot the correlation matrix with the lower triangle without symmetry
    plt.figure(figsize=(12, 8))
    sns.heatmap(correlation_matrix, annot=False, fmt=".2f", cmap="coolwarm", mask=mask, vmin=-1, vmax=1)
    ...
    def multivariate_linear_regression(x_train, y_train, x_test, y_test, lr=0.01, n_epochs=200000, patience=2000):
        ...
        ax.set_ylabel("Loss")
        ax.legend()
        plt.savefig(data_path + "/PCCprunedreg_loss.png", dpi=100)
        ...
        plt.savefig(data_path + "/PCCprunedpred_true.png", dpi=100)
    ...
    x = PCCpruned_reps_overtime.T.to_numpy() # encodings
