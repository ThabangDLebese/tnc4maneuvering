...
from tnc4maneuvering.models import WFClassifier, WFPCCEncoder, WFEncoder
...
def train(train_loader, valid_loader, classifier, lr, data_type, encoder=None, n_epochs=500, type='e2e', cv=0):
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig(os.path.join("./plots/%s" % data_type, "classification_pcc_%s_%d.png"%(type, cv)), dpi=100)
...
def run_test_pcc(data, tnc_lr, data_path, window_size, n_cross_val):
    ...
    for cv in range(n_cross_val):
        ...
        if data == f'{args.dataset}':
            ...
            # Create a new instance of the WFPcaEncoder with the desired num_pca_components
            tnc_encoder = WFEncoder(encoding_size=encoding_size).to(device)
            if not os.path.exists('./ckpt/one_ds/checkpoint_%d.pth.tar' % cv):
            ...
            # Save the encodings as a CSV file
            output_file = './ckpt/one_ds/one_dsPCCencodings_cv%d.csv' % cv
            save_latent_representations(tnc_encoder, test_loader, output_file)
            reduced_encodings = np.genfromtxt(output_file, delimiter=',')
            new_encoding_size = reduced_encodings.shape[1]

            # Create a new instance of WFPCCEncoder with the new reduced encoding size
            tnc_encoder = WFPCCEncoder(encoding_size=new_encoding_size, classify=False).to(device)
            tnc_encoder.load_state_dict(tnc_checkpoint['encoder_state_dict'])
            tnc_encoder.eval()

            tnc_classifier = WFClassifier(encoding_size=new_encoding_size, output_size=n_classes)
            tnc_model = torch.nn.Sequential(tnc_encoder, tnc_classifier).to(device)
            n_epochs = 20

        best_acc_tnc, best_auc_tnc, best_auprc_tnc = train(train_loader, valid_loader, tnc_classifier, tnc_lr, encoder=tnc_encoder, data_type=data, n_epochs=n_epochs, type='tnc', cv=cv)
...
if __name__ == '__main__':
    random.seed(1234)
    parser = argparse.ArgumentParser(description='Run PCC-pruning classification')
    ...
    f.close()
    run_test_pcc(data=args.dataset, tnc_lr=0.01, data_path=data_path, window_size=250, n_cross_val=args.cv)
